...
test_that("genome catalog is fixed-length, named, and versioned", {
  st <- knob_genome_stats(sim_small(1))
  expect_identical(st$version, "g6")
  p <- knob_genome_predictors(st)
...
test_that("reduced ABC set selects the three families plus size context", {
  p  <- knob_genome_predictors(knob_genome_stats(sim_small(2)))
  ab <- knob_abc_predictors(p)
...
test_that("no-exchange genomes score genuine zero derived sharing (not NA)", {
  p <- knob_genome_predictors(knob_genome_stats(sim_small(3, p_translocation = 0)))
...
test_that("exchange raises the cross-chromosome sharing family", {
  p0 <- knob_genome_predictors(knob_genome_stats(sim_small(4, p_translocation = 0)))
  p1 <- knob_genome_predictors(knob_genome_stats(
...
test_that("divergence filter and clock behave: older genome is more diverged", {
  young <- knob_genome_stats(sim_small(5))
  set.seed(5)
  older <- knob_genome_stats(sim_genome(K = 3L, target_sizes = rep(250L, 3L),
